...
{
    public partial class Scitani : Form
    {
        public Scitani()
        {
            InitializeComponent();
        ...
        public void Vypoceti()
        {
            ...
            {
                if (vypocti.VratImagSoucet() < 0)
                {
                    prubeh_a_vysledek.Text = "a + b = " + vypocti.VratRealSoucet().ToString() + " " + vypocti.VratImagSoucet().ToString() + "i ";
                    parsoval = true;
                }
                else
                {
        ...
        private void pustup_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show(vypocti.PostupRealScitani() + "\n" + vypocti.PostupImagScitani());
            }
        }

        private void button2_Click(object sender, EventArgs e)
        {
            MessageBox.Show("Sčítání Kalkulačka: Tato kalkulačka počítá s komplexními čísli jako v běžné matematice, takže s výrazy např. (5 + 6i), do levých Textboxů se píšou reálná čísla (ty bez íčka) a do prava imaginární (ty s íčkem), po zadání čísel klikni na tlačítko „Sečti“ aby se to sečetlo, se zápornými čísli si nelam hlavu, program vše zařídí za tebe, do Textboxu s výsledkem se ti ukáže pouze výsledek, pro postup budeš muset kliknout na tlačítko „Ukaž postup“");
        }
